...
- `时间`：返回当前服务器时间
- `说 <内容>`：复读指定文本
- `朗读 <内容>` / `念 <内容>`：把文字合成为语音并发回
- `生图 <提示词>` / `画图 <提示词>`：调用 MiniMax 文生图
- `图生图 <图片链接> | <提示词>`：调用 MiniMax 图生图
- 也支持两步交互：先发 `图生图 提示词`，再单独发图片
- 群聊里支持 MiniMax 随机插话：参考最近上下文，对具体消息进行暴躁阴阳风回复
- 群友发 B 站视频链接时，自动抓标题/简介等信息并结合上下文吐槽回复
- `新闻` / `要闻`：手动拉取 follow-builders 的 AI Builder X 要闻
- `日报总结` / `夜间总结`：手动拉取默认群最近 24 小时总结
- `订阅新闻` / `取消订阅新闻`：在当前群或私聊开关自动推送
- 支持默认定时推送新闻到指定群（当前默认 08:00，可配置）
- 支持默认定时推送最近 24 小时群聊总结到指定群（当前默认 00:30，可配置）
- 关键词轻响应：`你好` / `在吗` / `机器人`
- 已接入定时任务能力（apscheduler）
